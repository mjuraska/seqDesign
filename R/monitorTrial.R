...
#' @param lagTime a time point (in weeks) defining the per-protocol VE estimand of the type \code{effEst}, i.e., VE(\code{lagTime}--\code{stage1}). This VE estimand is also used in non-efficacy monitoring if \code{laggedMonitoring} equals \code{TRUE}. More specifically, if \code{nonEffTimes} is specified and \code{laggedMonitoring=TRUE}, then \code{nonEffTimes} will be interpreted as "lagged counts", events that occurred after the specified \code{lagTime} weeks of follow-up. It is typically chosen as the date of the last immunization or the date of the visit following the last immunization.
