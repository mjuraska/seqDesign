...
#' @param nonEffStartMethod a character string specifying the method used for determining when non-efficacy monitoring is to start. The default method of Freidlin, Korn, and Gray (2010) ("\code{FKG}") calculates the minimal pooled event count (pooled over the placebo and vaccine arm accruing events the fastest) such that a hazard-ratio-based VE point estimate of 0\% would result in declaring non-efficacy, i.e., the upper bound of the two-sided \eqn{(1-\code{alphaNoneff}) x 100\%} confidence interval for VE based on the asymptotic variance of the log-rank statistic equals the non-efficacy threshold specified as component \code{upperVEnonEff} in the list \code{nonEffStartParams}. If this list component is left unspecified, the argument \code{upperVEnonEff} is used as the non-efficacy threshold. The alternative method ("\code{fixed}") starts non-efficacy monitoring at a fixed pooled event count (pooled over the placebo and vaccine arm accruing events the fastest) specified by component \code{N1} in the list \code{nonEffStartParams}.
...
#' @param nonEffCohorts a named list specifying all cohorts (for both timing and analysis) used in non-efficacy monitoring. The required list components characterize the 'timing cohort,' i.e., the cohort events in which determine the analysis timepoints in an event-driven design (components \code{times}, \code{timeUnit}, and \code{timingCohort}), and the analysis cohort(s), i.e., the cohort(s) in which inference is made about the null hypothesis of non-efficacy (\code{cohort1} is required, and \code{cohort2}, etc. are optional for additional analysis cohorts if, e.g., non-efficacy monitoring is conducted in both the ITT and per-protocol cohorts). As for the timing cohort, \code{times} specifies the analysis timepoints in terms of event counts (\eqn{\code{timeUnit} = \code{"counts}} is the only implemented option). \code{timingCohort} is a list characterizing the timing cohort by components \code{lagTime}, a lag time (in weeks) that controls event inclusion for timing (it no lag is desired, it can be set to \code{NULL} or 0), and \code{cohortInd}, a character string naming an indicator variable included in the data frames outputted by \code{\link{simTrial}}, which also controls event inclusion for the purpose of determining analysis timepoints. Top-level list components \code{cohort1}, \code{cohort2}, etc. are each a list that must contain a component named \code{estimand}, which can take on values \code{"cox"}, \code{"cuminc"}, or \code{"combined"}. Optional list components in \code{cohort1}, \code{cohort2}, etc. are \code{lagTime}, \code{cohortInd}, \code{nullVE}, and \code{nominalAlphas}. \code{lagTime} specifies a lag time (in weeks) that controls event inclusion for the analysis cohort. If no lag time is desired, then this component can be ignored, set to \code{NULL}, or set to 0. \code{cohortInd} is a character string naming an indicator variable included in the data frames outputted by \code{\link{simTrial}} to be used to subset participants into the analysis cohort. This component allows inclusion of, e.g., a "per-protocol" variable (e.g., by setting \code{cohortInd} to \code{"pp1"}). \code{nullVE} specifies the one-sided null hypothesis of non-efficacy as \eqn{H_0: VE \geq \code{nullVE}}. \code{nominalAlphas} specifies nominal significance levels in a two-arm event-driven trial design.
