...
post6moMonitor <- TRUE   # should post-6 months non-efficacy monitoring be applied?
...
print(xtable(res, 
             ...
             digits=1, 
             caption=paste0("Probabilities ($\\times 100$) of reaching each possible trial monitoring outcome",ifelse(!is.null(lowerVEuncPower)," and unconditional power ($\\times 100$) to reject the specified null hypothesis","")," for a study design with 1 vaccine arm with ",N.pla," placebo recipients and ",N.vax," vaccine recipients")
             ), 
...
text(0.37, 0.5, 
     paste0("N=",N.pla,":",N.vax," placebo:vaccine group\n",infRate*100,"% annual placebo group incidence\n5% annual dropout\nVE halved in first 6 months\n",ifelse(estimand=="combined","Cox & cum inc-based ",ifelse(estimand=="cuminc","Cum inc ","Cox model ")),ifelse(post6moMonitor,"non-eff w/ post-6 mo monitoring\n","non-eff monitoring\n"),ifelse(estimand=="cox","Cox model high eff & unconditional power","Cum inc high eff & unconditional power"),"\n18-month accrual; average 309 per month"), 
     cex=1, pos=4)
cat("\\caption{Probabilities of reaching each possible trial monitoring outcome",ifelse(!is.null(lowerVEuncPower),", and unconditional power to reject the specified null hypothesis","")," for a study design with 1 vaccine arm with ",N.pla," placebo recipients and ",N.vax," vaccine recipients}", sep="")
@
\end{center}
\end{figure}
...
harmData <- read.csv(file.path(srcDir, paste0("harmBounds_N=100_alphaPerTest=0.0106_pVacc=",round(N.vax/(N.pla+N.vax),2),".csv")))
harmData <- subset(harmData, N>=10)
par(mar=c(5.5,5,1,1), las=1, cex.axis=1.1, cex.lab=1.2)
plot(-10,0, xlim=c(10,100), ylim=c(9,60), xaxt="n", yaxt="n", xlab="", ylab="", type="n")
...
segments(x0=80, y0=8, y1=16, lwd=2, lty="dashed", col="darkorange")
segments(x0=80, y0=30, y1=harmData$V[harmData$N==80], lwd=2, lty="dashed", col="darkorange")
segments(x0=10, x1=80, y0=harmData$V[harmData$N==80], lwd=2, lty="dashed", col="darkorange")
text(66, 23, "Potential Harm\nStopping\nBoundaries", adj=0, cex=1.1, font=2, col="blue")
@
\end{center}
\end{figure}
...
nInf <- seq(firstNoneffCnt,250,by=20)
...
# calculate the MC average number of infections (pooled over placebo + 1 vaccine arm) triggering the second high efficacy IA for trials with average VE=20%
secondHigheffCnt <- round(colMeans(do.call("rbind", lapply(out, function(trial){ trial[[1]]$highEffCnt })))[2],0)
nInfH <- c(firstNoneffCnt+4*20, secondHigheffCnt)
...
par(mar=c(5.5,5,1,1), las=1, cex.axis=1.1, cex.lab=1.2)
plot(-10,0, xlim=c(0,250), ylim=c(-10,20), xaxt="n", xlab="", ylab="", type="n")
...
axis(side=1, at=77)
axis(side=2, at=seq(-10,20,by=5))
axis(side=3, at=c(0,50,77,100,150,200,250), labels=FALSE)
axis(side=4, at=seq(-10,20,by=5), labels=FALSE)
...
\begin{figure}[H]
\begin{center}
<<label=plotHighEffStopBounds, echo=FALSE, fig.width=5, fig.height=3.8>>=
par(mar=c(5.5,5,1,1), las=1, cex.axis=1.1, cex.lab=1.2)
plot(-10,0, xlim=c(0,250), ylim=c(65,90), xaxt="n", xlab="", ylab="", type="n")
mtext("Total Number of Infections\n(Placebo + 1 Vaccine Arm)", side=1, line=3.5, cex=1.2)
mtext("Vaccine Efficacy (%)", side=2, line=2.5, cex=1.2, las=0)
axis(side=1, at=c(0,50,100,150,200,250))
axis(side=3, at=c(0,50,100,150,200,250), labels=FALSE)
